...
const getANumberOfOperations = async (req, res) => {
    if(!req.query.page || !req.query.limit) return res.status(400).send({message:'Missing page number or limit in the url of the request'});

    const page = parseInt(req.query.page);
    const limit = parseInt(req.query.limit);

    if(!page || !limit) return res.status(400).send({message:'Page and limit values must be integers greater than 0'});
